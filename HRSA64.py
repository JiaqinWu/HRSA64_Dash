...
USERS = {
    ...
    'tmartin2@hrsa.gov':{
        "HRSA": {"password": "HRSA2024!", "name": "Tamika Martin"}
    }
}
...
if st.session_state.role is None:
    ...
    role = st.selectbox(
        "Select your role",
        ["Requester", "Coordinator", "Assignee/Staff","Research Assistant", "HRSA"],
        index=None,
...
# --- Show view based on role
    ...
    # --- Coordinator, Staff, or HRSA: Require login
    elif st.session_state.role in ["Coordinator", "Assignee/Staff", "Research Assistant", "HRSA"]:
        if not st.session_state.authenticated:
            ...
            if st.session_state.role == "Coordinator":
                ...
                # HRSA Monitoring Dashboard
                st.markdown("<hr style='margin:2em 0; border:1px solid #dee2e6;'>", unsafe_allow_html=True)
                with st.expander("📊 **HRSA MONITORING DASHBOARD**"):
                    st.markdown("""
                        <div style='background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius: 20px; box-shadow: 0 8px 32px rgba(102, 126, 234, 0.3); padding: 2em 1.5em 1.5em 1.5em; margin-bottom: 2em; margin-top: 1em;'>
                            <div style='color: white; font-family: "Segoe UI", "Arial", sans-serif; font-weight: 800; font-size: 1.6em; margin-bottom: 0.5em; text-align: center;'>
                                📊 HRSA Monitoring Dashboard
                            ...
                            <div style='color: rgba(255,255,255,0.9); font-size: 1.1em; margin-bottom: 0.8em; text-align: center; line-height: 1.4;'>
                                Monitor requests, interactions, and deliverables across all jurisdictions. Drill down from executive overview to detailed request views.
                            </div>
                        </div>
                    """, unsafe_allow_html=True)

                    # Level 1: Executive Overview
                    st.markdown("### Level 1 – Executive Overview")
                    
                    # Calculate KPIs
                    total_requests = df['Ticket ID'].nunique()
                    total_interactions = len(df_int) if not df_int.empty else 0
                    total_deliverables = len(df_del) if not df_del.empty else 0
                    
                    # Display KPIs
                    col1, col2, col3 = st.columns(3)
                    col1.metric("Total Requests", millify(total_requests, precision=2))
                    col2.metric("Total Interactions", millify(total_interactions, precision=2))
                    col3.metric("Total Deliverables", millify(total_deliverables, precision=2))
                    style_metric_cards(border_left_color="#DBF227")
                    
                    # Bar chart by jurisdiction
                    st.markdown("#### Requests by Jurisdiction")
                    if not df.empty and 'Jurisdiction' in df.columns:
                        jurisdiction_counts = df['Jurisdiction'].value_counts().reset_index()
                        jurisdiction_counts.columns = ['Jurisdiction', 'Count']
                        jurisdiction_counts = jurisdiction_counts.sort_values('Count', ascending=False)
                        
                        bar_chart = alt.Chart(jurisdiction_counts).mark_bar().encode(
                            x=alt.X('Jurisdiction', sort='-y', axis=alt.Axis(labelAngle=-45)),
                            y=alt.Y('Count', title='Number of Requests'),
                    ...
                    # Optional: Simple map (placeholder - can be enhanced with actual mapping library)
                    st.markdown("#### Geographic Distribution")
                    st.info("Map visualization can be added here with a mapping library (e.g., folium, plotly).")
                    
                    st.markdown("<hr style='margin:2em 0; border:1px solid #dee2e6;'>", unsafe_allow_html=True)
                    
                    # Level 2: Jurisdiction View
                    st.markdown("### Level 2 – Jurisdiction View")
                    
                    # Get unique jurisdictions
                    if not df.empty and 'Jurisdiction' in df.columns:
                        unique_jurisdictions = sorted([j for j in df['Jurisdiction'].unique() if pd.notna(j) and str(j).strip() != ''])
                        
                        if unique_jurisdictions:
                            selected_jurisdiction = st.selectbox(
                                "Select a Jurisdiction",
                                options=unique_jurisdictions,
                                index=None,
                            ...
                            if selected_jurisdiction:
                                ...
                                jurisdiction_requests = df[df['Jurisdiction'] == selected_jurisdiction].copy()
                                
                                # Get request IDs
                                request_ids = jurisdiction_requests['Ticket ID'].unique().tolist()
                                
                                # Count interactions for this jurisdiction
                                # Match by Ticket ID or Jurisdiction
                                jurisdiction_interactions = df_int[
                                    (df_int['Ticket ID'].isin(request_ids)) | 
                                    (df_int['Jurisdiction'] == selected_jurisdiction)
                                ].copy()
                                interaction_count = len(jurisdiction_interactions)
                                
                                # Count deliverables for this jurisdiction
                                jurisdiction_deliverables = df_del[
                                    df_del['Ticket ID'].isin(request_ids)
                                ].copy()
                                deliverable_count = len(jurisdiction_deliverables)
                                
                                # Display summary metrics
                                col1, col2, col3, col4 = st.columns(4)
                                col1.metric("Request IDs", len(request_ids))
                                col2.metric("Interactions", interaction_count)
                                col3.metric("Deliverables", deliverable_count)
                                col4.metric("Status Distribution", f"{jurisdiction_requests['Status'].value_counts().to_dict()}")
                                
                                # Summary table
                                st.markdown("#### Summary Table")
                                summary_cols = ['Ticket ID', 'Status', 'Organization', 'Name', 'Focus Area', 'TA Type', 'Assigned Coach', 'Submit Date']
                                available_cols = [col for col in summary_cols if col in jurisdiction_requests.columns]
                                
                                # Format dates
                                if 'Submit Date' in jurisdiction_requests.columns:
                                    jurisdiction_requests['Submit Date'] = pd.to_datetime(jurisdiction_requests['Submit Date'], errors='coerce')
                                    jurisdiction_requests_display = jurisdiction_requests.copy()
                                    jurisdiction_requests_display['Submit Date'] = jurisdiction_requests_display['Submit Date'].dt.strftime("%Y-%m-%d")
                                else:
                                    jurisdiction_requests_display = jurisdiction_requests.copy()
                                
                                st.dataframe(
                                    jurisdiction_requests_display[available_cols].sort_values('Submit Date' if 'Submit Date' in available_cols else 'Ticket ID', ascending=False),
                                ...
                                if 'Submit Date' in jurisdiction_requests.columns:
                                    jurisdiction_requests['Submit Date'] = pd.to_datetime(jurisdiction_requests['Submit Date'], errors='coerce')
                                    jurisdiction_requests['Month'] = jurisdiction_requests['Submit Date'].dt.to_period('M').astype(str)
                                    
                                    monthly_counts = jurisdiction_requests.groupby('Month').size().reset_index()
                                    ...
                                    st.info("Submit Date column not available for trend analysis.")
                                
                                st.markdown("<hr style='margin:2em 0; border:1px solid #dee2e6;'>", unsafe_allow_html=True)
                                
                                # Level 3: Request Detail View
                                st.markdown("### Level 3 – Request Detail View")
                                
                                if request_ids:
                                    selected_request_id = st.selectbox(
                                        "Select a Request ID",
                                        options=sorted(request_ids, key=lambda x: str(x)),
                                        index=None,
                                    ...
                                    if selected_request_id:
                                        # Get request details
                                        request_details = df[df['Ticket ID'] == selected_request_id].iloc[0] if not df[df['Ticket ID'] == selected_request_id].empty else None
                                        
                                        if request_details is not None:
                                            # Display request information
                                            st.markdown("#### Request Information")
                                            info_cols = ['Ticket ID', 'Jurisdiction', 'Organization', 'Name', 'Status', 'Focus Area', 'TA Type', 'Assigned Coach', 'Submit Date', 'Targeted Due Date']
                                            available_info_cols = [col for col in info_cols if col in request_details.keys()]
                                            
                                            request_info_dict = {col: request_details[col] for col in available_info_cols}
                                            request_info_df = pd.DataFrame([request_info_dict])
                                            
                                            # Format dates
                                            for date_col in ['Submit Date', 'Targeted Due Date']:
                                                if date_col in request_info_df.columns:
                                                    request_info_df[date_col] = pd.to_datetime(request_info_df[date_col], errors='coerce')
                                                    request_info_df[date_col] = request_info_df[date_col].dt.strftime("%Y-%m-%d")
                                            
                                            st.dataframe(request_info_df, use_container_width=True)
                                            
                                            # Full interaction history
                                            st.markdown("#### Full Interaction History")
                                            interactions_for_request = df_int[
                                                df_int['Ticket ID'].astype(str) == str(selected_request_id)
                                            ].copy()
                                            
                                            if not interactions_for_request.empty:
                                                # Format dates
                                                if 'Date of Interaction' in interactions_for_request.columns:
                                                    interactions_for_request['Date of Interaction'] = pd.to_datetime(interactions_for_request['Date of Interaction'], errors='coerce')
                                                    interactions_for_request = interactions_for_request.sort_values('Date of Interaction', ascending=False)
                                                    interactions_for_request['Date of Interaction'] = interactions_for_request['Date of Interaction'].dt.strftime("%Y-%m-%d")
                                                
                                                # Display columns (exclude internal columns)
                                                interaction_display_cols = [col for col in interactions_for_request.columns 
                                                                           if col not in ['Submitted By', 'Submission Date']]
                                                st.dataframe(
                                                    interactions_for_request[interaction_display_cols].reset_index(drop=True),
                                                    use_container_width=True
                                                )
                                                
                                                # Export interaction history
                                                csv_interactions = interactions_for_request[interaction_display_cols].to_csv(index=False)
                                                st.download_button(
                                                    label="📥 Download Interaction History (CSV)",
                                                    data=csv_interactions,
                                                    file_name=f"interactions_{selected_request_id}.csv",
                                                    mime="text/csv"
                                                )
                                            else:
                                                st.info("No interactions recorded for this request.")
                                            
                                            # Deliverables list
                                            st.markdown("#### Deliverables List")
                                            deliverables_for_request = df_del[
                                                df_del['Ticket ID'].astype(str) == str(selected_request_id)
                                            ].copy()
                                            
                                            if not deliverables_for_request.empty:
                                                # Format dates if available
                                                date_cols_del = [col for col in deliverables_for_request.columns if 'Date' in col or 'date' in col.lower()]
                                                # Format dates if available
                                                for date_col in date_cols_del:
                                                    deliverables_for_request[date_col] = pd.to_datetime(deliverables_for_request[date_col], errors='coerce')
                                                    deliverables_for_request[date_col] = deliverables_for_request[date_col].dt.strftime("%Y-%m-%d")
                                                
                                                # Sort by date if available
                                                if date_cols_del:
                                                    deliverables_for_request = deliverables_for_request.sort_values(date_cols_del[0], ascending=False)
                                                
                                                # Display columns (exclude internal columns)
                                                deliverable_display_cols = [col for col in deliverables_for_request.columns 
                                                                          if col not in ['Submitted By', 'Submission Date']]
                                                st.dataframe(
                                                    deliverables_for_request[deliverable_display_cols].reset_index(drop=True),
                                                    use_container_width=True
                                                )
                                                
                                                # Export deliverables
                                                csv_deliverables = deliverables_for_request[deliverable_display_cols].to_csv(index=False)
                                                st.download_button(
                                                    label="📥 Download Deliverables List (CSV)",
                                                    data=csv_deliverables,
                                                    file_name=f"deliverables_{selected_request_id}.csv",
                                                    mime="text/csv"
                                                )
                                            else:
                                                st.info("No deliverables recorded for this request.")
                                            
                                            # Combined exportable table
                                            st.markdown("#### Exportable Combined Table")
                                            if not interactions_for_request.empty or not deliverables_for_request.empty:
                                                # Create combined export
                                                export_data = []
                                                
                                                # Add interactions
                                                for _, row in interactions_for_request.iterrows():
                                                    export_data.append({
                                                        'Type': 'Interaction',
                                                        'Date': row.get('Date of Interaction', ''),
                                                        'Description': row.get('Short Summary', ''),
                                                        'Type Detail': row.get('Type of Interaction', ''),
                                                        'Document': row.get('Document', '')
                                                    })
                                                
                                                # Add deliverables
                                                for _, row in deliverables_for_request.iterrows():
                                                    date_col = next((col for col in row.index if 'Date' in col), '')
                                                    export_data.append({
                                                        'Type': 'Deliverable',
                                                        'Date': row.get(date_col, '') if date_col else '',
                                                        'Description': row.get('Deliverable Name', row.get('Description', '')),
                                                        'Type Detail': row.get('Deliverable Type', ''),
                                                        'Document': row.get('Document', '')
                                                    })
                                                
                                                if export_data:
                                                    export_df = pd.DataFrame(export_data)
                                                    # Sort by date (handle empty dates)
                                                    export_df['Date_Sort'] = pd.to_datetime(export_df['Date'], errors='coerce')
                                                    export_df = export_df.sort_values('Date_Sort', ascending=False, na_position='last')
                                                    export_df = export_df.drop('Date_Sort', axis=1)
                                                    st.dataframe(export_df, use_container_width=True)
                                                    
                                                    csv_combined = export_df.to_csv(index=False)
                                                    st.download_button(
                                                        label="📥 Download Combined History (CSV)",
                                                        data=csv_combined,
                                                        file_name=f"combined_history_{selected_request_id}.csv",
                                                        mime="text/csv"
                                                    )
                                        else:
                                            st.warning(f"Request {selected_request_id} not found.")
                                else:
                                    st.info("No request IDs available for this jurisdiction.")
                        else:
                            st.info("No jurisdictions available in the data.")
                    else:
                        st.info("Jurisdiction data not available.")
...
GU-TAP System
            ...
            elif st.session_state.role == "HRSA":
                user_info = USERS.get(st.session_state.user_email)
                hrsa_name = user_info["HRSA"]["name"] if user_info and "HRSA" in user_info else "HRSA User"
                st.markdown(
                    """
                    <div style='
                        display: flex;
                ...
                # HRSA Monitoring Dashboard Content
                st.markdown("""
                    <div style='background: linear-gradient(135deg, #667eea 0%, #764ba2 100%); border-radius: 20px; box-shadow: 0 8px 32px rgba(102, 126, 234, 0.3); padding: 2em 1.5em 1.5em 1.5em; margin-bottom: 2em; margin-top: 1em;'>
                        <div style='color: white; font-family: "Segoe UI", "Arial", sans-serif; font-weight: 800; font-size: 1.6em; margin-bottom: 0.5em; text-align: center;'>
                            📊 HRSA Monitoring Dashboard
